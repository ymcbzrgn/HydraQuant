  ...
  <div class="card p-3 sm:p-4">
    <div class="flex justify-between items-center mb-4">
      <h2 class="text-lg sm:text-xl font-bold flex items-center gap-2">
        <i class="pi pi-shield"></i> Daily Loss Limit
      </h2>
    ...
    <div v-if="aiStore.risk" class="flex flex-col gap-4 sm:gap-6">
      <!-- Budget Overview -->
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
        <div class="bg-gray-50 dark:bg-gray-800 p-3 sm:p-4 rounded-lg border dark:border-gray-700">
          <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Daily Budget</div>
          <div class="text-xl sm:text-2xl font-bold">${{ aiStore.risk.daily_budget.toFixed(2) }}</div>
        </div>
        <div class="bg-gray-50 dark:bg-gray-800 p-3 sm:p-4 rounded-lg border dark:border-gray-700">
          <div class="text-sm text-gray-600 dark:text-gray-400 mb-1">Active Positions</div>
          <div class="text-xl sm:text-2xl font-bold text-blue-500 dark:text-blue-400">{{ aiStore.risk.active_positions }}</div>
        </div>
      </div>
      ...
      <div>
        <div class="flex justify-between text-sm mb-2">
          <span class="font-medium text-gray-700 dark:text-gray-300">Budget Consumed</span>
        ...
        <div v-if="aiStore.risk.utilization_pct >= 100" class="mt-2 text-xs text-red-500 dark:text-red-400 font-bold flex items-center gap-1">
          <i class="pi pi-exclamation-triangle"></i> Daily budget exceeded! New entries paused.
        </div>
      </div>

      <!-- 7-Day History -->
      <div class="mt-2">
        <h3 class="text-sm font-semibold mb-3 text-gray-600 dark:text-gray-400">7-Day Utilization Trend</h3>
        <div class="flex items-end gap-2 h-20 sm:h-24 border-b border-l p-2 pt-0 dark:border-gray-700">
          <div v-for="(day, i) in riskHistoryMocks" :key="i" class="w-full flex flex-col justify-end items-center group relative cursor-pointer">
            ...
            <div class="opacity-0 group-hover:opacity-100 absolute -top-6 sm:-top-8 bg-gray-950 dark:bg-gray-600 text-white text-xs p-1 rounded z-10 whitespace-nowrap">
              {{ day.date }}: {{ day.pct.toFixed(0) }}%
            </div>
            <div class="text-[9px] sm:text-[10px] text-gray-500 dark:text-gray-400 mt-1">{{ day.dayLabel }}</div>
          </div>
        </div>
      </div>
    </div>
